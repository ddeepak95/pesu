...
export interface ClassStudentInvite {
  id: string;
  class_id: string;
  created_by: string;
  expires_at: string;
...
export async function createStudentInvite(params: {
  ...
  const { data, error } = await supabase.rpc("create_student_invite", args);
...
export async function listStudentInvites(classDbId: string): Promise<ClassStudentInvite[]> {
  const supabase = createClient();
  ...
  if (error) throw error;
  return (data || []) as ClassStudentInvite[];
}

export async function acceptStudentInvite(token: string): Promise<string> {
  const supabase = createClient();
